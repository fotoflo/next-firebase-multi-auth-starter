...
export default function FirebaseAdapter(
  ...
  const findVerificationTokenDoc = (key: string) =>
    verificationTokenCollectionRef.doc(key);
  const findCustomTokenDoc = (sessionToken: string) =>
    customTokenCollectionRef.doc(sessionToken);
  ...
  return {
    ...
    async deleteSession(sessionToken) {
      ...
      const userRef = await findUserDoc(
        sessionRef.data().userId as string
      ).get();
    ...
    async createVerificationToken(data) {
      // need test
      const verificationTokenRef = await verificationTokenCollectionRef.add(
        data
      );
    ...
    async useVerificationToken({ identifier, token }) {
      // need test
      const q = verificationTokenCollectionRef
